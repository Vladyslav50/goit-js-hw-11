...
// Імпортуємо функцію для виконання HTTP-запитів з окремого файлу api.js
import { searchImages } from './api';

// Імпортуємо функцію для створення DOM-елементів з окремого файлу helpers.js
import { createImageCard } from './helpers';
...
// Отримуємо посилання на форму пошуку, галерею та кнопку "Load more"
...
searchForm.addEventListener('submit', async event => {
  ...
  // Перевіряємо, чи не є поле пошуку пустим перед відправленням запиту
  if (!query) {
    return;
  }
...
async function performSearch(query, page) {
  try {
    // Виконуємо пошук зображень та отримуємо результат з окремої функції searchImages
    ...
    if (page === 1) {
      ...
      // Відображаємо повідомлення про кількість знайдених зображень, якщо totalHits більше 0
      if (totalHits > 0) {
    ...
    // Перевіряємо, чи маємо ще зображення для завантаження
    ...
    Notiflix.Notify.warning(`Error performing search: ${error}.`);
  }
}
...
//   // Перевірка, чи не є рядок пошуку пустим
//   if (query === '') {
//     return;
//   }
